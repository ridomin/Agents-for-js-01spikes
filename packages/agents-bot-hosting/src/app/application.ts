...
import { Activity, ActivityTypes, ChannelInfo, ConversationReference, validateValueActionFeedbackLoopData, validateValueAction, validateValueActionName, validateValueContinuation } from '@microsoft/agents-bot-activity'
import { ReadReceiptInfo } from '../teams/readReceipInfo'
...
export class Application<TState extends TurnState = TurnState> {
  ...
  public fileConsentAccept (
    ...
    const selector = (context: TurnContext): Promise<boolean> => {
      const action = validateValueAction(context.activity.value)
  ...
  public handoff (handler: (context: TurnContext, state: TState, continuation: string) => Promise<void>): this {
    ...
    const handlerWrapper = async (context: TurnContext, state: TState) => {
      await handler(context, state, validateValueContinuation(context.activity.value))
      await context.sendActivity(Activity.fromObject({
        type: ActivityTypes.InvokeResponse,
        value: { status: 200 }
      }))
  ...
  public feedbackLoop (
    ...
    const selector = (context: TurnContext): Promise<boolean> => {
      const actionName = validateValueActionName(context.activity.value)
      return Promise.resolve(
        context.activity.type === ActivityTypes.Invoke &&
    ...
    const handlerWrapper = async (context: TurnContext, state: TState) => {
      const actionValue = validateValueActionFeedbackLoopData(context.activity.value)
